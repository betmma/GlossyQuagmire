...
function SetFont(size,forcedFont)
    if forcedFont then
        local font=getFont(forcedFont,size)
...
---to rjust score text and add zero as last digit if needed.
...
---@param addZero? boolean add 0 as last digit, for all gained score texts
function scoreToString(value,rjust,addZero)
    local scoreString
    if rjust then
