                ...
                func=function(self, boss)
                    ...
                    local function releaseUpdate(self)
                        ...
                        elseif t>30 and t<100 then
                            self.size=math.lerp(self.size,2,0.03)
                        end
                        if t>0 and t<60 then
                            self.kinematicState.speed=self.kinematicState.speed+2
                        end
                    end
                    local function slash(pos1,pos2,releaseFrame,color,side)
                        local dist=geo:distanceRef(pos1,pos2)
                    ...
                    local function slashAtDir(dir,dir2,releaseFrame,color,side,num)
                        local posa,dira=geo:rThetaGoRef(posp,100,dir)
                        local pos1=geo:rThetaGoRef(posa,500,dira+dir2)
                        local pos2=geo:rThetaGoRef(posa,-2500,dira+dir2)
                        slashBatch(pos1,pos2,releaseFrame,color,side,num)
                    end
                    local colors={'red','orange','purple','magenta'}
                    for i=1,8 do
                        SFX:play('enemyShot')
                        slashAtDir(dir0-math.pi/2,math.pi/2*0.8*math.mod2Sign(i),60,colors[(i-1)%4+1],math.mod2Sign(i),3)
                        wait(60)
                        SFX:play('enemyShot')
                        wait(300)
                    end
