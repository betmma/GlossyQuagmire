...
vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords) {
    ...
    float mask = uv.y*0.5+(gray-averageGrayscale); // Transition based on grayscale value and progress
    float alpha = 1.0-smoothstep(progress*1.1-0.1, progress*1.1, mask);
